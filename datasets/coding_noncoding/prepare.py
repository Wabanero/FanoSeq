"""Normalize a curated coding/noncoding FASTA and metadata table.

This script intentionally does not download GENCODE files. Curators should build
the coding/noncoding windows from the pinned source release, then use this script
to validate columns, copy prepared inputs, and record hashes.
"""
...
def _validate_metadata(rows: list[dict[str, str]]) -> None:
    ...
    for row in rows:
        ...
        if label not in ALLOWED_LABELS:
            raise SystemExit(f"Unsupported label for {sequence_id}: {label}")
